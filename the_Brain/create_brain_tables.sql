...
-- Core conventions apply to all entities with category-specific extensions

-- 1. Core Entity Table (base table for all entities)
CREATE TABLE IF NOT EXISTS core_entities (
    id VARCHAR PRIMARY KEY,
    type VARCHAR NOT NULL CHECK (type IN ('person', 'organization', 'place', 'event', 'conversation', 'task', 'project', 'object', 'media', 'idea', 'knowledge', 'preference', 'routine', 'health', 'travel', 'transaction', 'device_session', 'time_period')),
...
-- 2. Context Table (reusable context referenced by context_id)
...
CREATE TABLE IF NOT EXISTS persons (
    id VARCHAR PRIMARY KEY REFERENCES core_entities(id) ON DELETE CASCADE,
...
-- 7. Conversation/Message entities
...
-- 19. Device/App Session entities
...
CREATE INDEX IF NOT EXISTS idx_core_entities_tags ON core_entities USING GIN(tags);
CREATE INDEX IF NOT EXISTS idx_core_entities_salience ON core_entities(salience);
